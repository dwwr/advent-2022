...
const scoreRoundForDesiredOutcome = (round: string): number => {
  ...
  const myMove = Object.keys(outcomeMap[oppMove]).find(
    (move) => outcomeMap[oppMove][move] === desiredOutcome
  ) as string
  const myScore = moveScores[myMove]
  return outcomeScores[desiredOutcome] + myScore
}

const scoreStrategyGuide = (
  input: string,
  desiredOutcomes?: boolean
): number => {
  const rounds = generateRoundsFromInput(input)
  const totalScore = rounds.reduce((total, round) => {
    const roundScore = desiredOutcomes
      ? scoreRoundForDesiredOutcome(round)
      : scoreRound(round)
